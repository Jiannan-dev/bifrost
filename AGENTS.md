...
| Provider-independent official pricing and model-info fallback | Custom/case-variant providers otherwise record zero cost and return incomplete model metadata even when the synced datasheet has a first-party row. Keeps the upstream provider/model lookup first, then uses one shared conservative publisher fallback for pricing and model-info reads; normalizes datasheet modality fields into the existing model architecture response. | Upstream resolves case-variant built-in providers and custom-provider GPT/Claude/DeepSeek/GLM/Grok/Gemini/Mistral/Codestral/Cohere models against the synced first-party datasheet for both billing and complete model-info lookups while preserving actual-provider override scope and wire-model keys | `go test ./modelcatalog/datasheet -run 'Test(OfficialPricingFallback|GetPricingEntryForModel|EntryUnmarshalMaps)'`, `go test ./plugins/logging -run TestCalculateCostForLogUsesOfficialPricingFallbackWithCacheTokens`, `go test ./bifrost-http/handlers -run TestEnrichListModelsResponseFallsBackToOfficialPublisherMetadata`, plus provider harness folder 93 (`OpenAI/gpt-4o-mini` returns positive `usage.cost`). |
| Claude Code Anthropic thinking replay omits synthetic reasoning item ids | Claude Code echoes unmarked `thinking` / `redacted_thinking` blocks. Bifrost minted `rs_*` placeholders on convert-to-Responses; OpenCode Go `/v1/responses` (`store:false`) treated them as server-side handles and 400d `Referenced reasoning item ... was not found or has expired`. Recover an id only from `ExtractReasoningItemID`; otherwise leave it nil. Fail-soft strips leftover reasoning ids (including thinking-only items with no `ResponsesReasoning`) after that 400 and retries once without backoff. OpenCode egress also drops reasoning ids. Does **not** enable id embedding for OpenCode, does **not** set `BifrostContextKeyBypassSemanticCache`, and does **not** touch `prompt_cache_key*` (prefix-cache hygiene stays the other fork feature). | maximhq stops minting placeholder `rs_*` on unmarked Anthropic thinking, fail-softs the OpenCode/store:false item-id 400 by stripping ids on **all** reasoning-typed items (not only `ResponsesReasoning != nil`), and does not embed OpenCode ids for replay | `go test ./providers/anthropic -run 'TestConvertAnthropicContentBlocks_Genuine\|TestConvertAnthropicMessages_UnmarkedThinkingOmitsItemID\|TestBifrostAnthropicToOpenAI_RedactedThinkingReplayPreservesID'`, `go test . -run 'Test(IsReasoningItemIDRejection\|StripResponsesReasoningItemIDs\|ExecuteRequestWithRetries_StripsReasoningItemIDs\|ExecuteRequestWithRetries_ReasoningItemIDErrorDoesNotStripCiphertext\|ExecuteRequestWithRetries_StripsEncryptedContentAndRetries)'`, `go test ./providers/openai -run TestToOpenAIResponsesRequest_DropsReasoningItemIDsForOpenCode`, `go test ./providers/utils -run TestShouldEmbedReasoningItemID`. Harness exemption: public collection has no OpenCode Go / gpt-oss Responses host that forwards reasoning content blocks, so a live pin cannot see a minted `rs_*` on the wire; Go tests cover conversion, fail-soft, and egress. Live check is a Claude Code 2-turn tool call on `opencode-go/muse-spark-...` after deploy. |
| Claude Code Anthropic conversion mints deterministic function_call item ids | Anthropic→Responses conversion used to mint a fresh `fc_*` + 50 random chars as the `function_call` **item** `id` on every history replay (`call_id` stays the stable Anthropic `tool_use` id). OpenCode DeepSeek prefix cache is exact-match and stops at the first reminted `fc_*` (~2k tokens; cache ~1%). An earlier egress-only omit on OpenCode left logs reminting and live cache at ~1%. Conversion now derives `fc_` + sha256(`call_id`) hex[:50]. OpenCode egress still drops reasoning ids (400) but **keeps** these function_call ids. Does **not** strip `call_id`, does **not** set `BifrostContextKeyBypassSemanticCache`, and does **not** touch `prompt_cache_key*`. | maximhq stops minting random `fc_*` on Anthropic tool_use replay (deterministic from `call_id`, or omit only when that is proven cache-stable on OpenCode DeepSeek) | `go test ./providers/anthropic -run TestConvertAnthropicToolUse_FunctionCallItemIDIsStable`, `go test ./providers/openai -run 'TestToOpenAIResponsesRequest_KeepsFunctionCallItemIDsForOpenCode\|TestToOpenAIResponsesRequest_DropsReasoningItemIDsForOpenCode'`. Harness exemption: public collection has no OpenCode Go Responses host. Live check: after deploy, Claude Code multi-turn on `opencode-go/deepseek-v4-flash` must show the **same** `function_call.id` for the same `call_id` in `logs.db`, and `cached_read` must grow past ~2k. muse-spark 2-turn tool call must still succeed. |
...
### Claude Code Anthropic thinking replay omits synthetic reasoning item ids (fork feature)

Lets Claude Code finish a tool-call turn against official OpenCode Go (`opencode-go/...` over `/anthropic/v1/messages` → `/v1/responses`). Added because unmarked Anthropic `thinking` / `redacted_thinking` blocks have no OpenAI-issued item id, and Bifrost used to mint `rs_*` + 50 random chars. OpenCode Go runs `store: false`, looks that id up as a server-side handle, and 400s `Referenced reasoning item ... was not found or has expired` on the second turn. Upstream replacement: see feature matrix row above. Community PR maximhq/bifrost#6460 is **not** the replacement: it is unmerged, and its strip only touches items with `ResponsesReasoning != nil`, which misses Claude Code thinking-only items.

**Where it runs:**

1. Anthropic → Responses ingress (`convertAnthropicContentBlocksToResponsesMessages` and the grouped twin): recover an id only via `ExtractReasoningItemID`. Otherwise `ID` stays nil. Payload bytes are unchanged.
2. Retry loop (`executeRequestWithRetries`): on that 400 only, clone the input, nil ids on every reasoning-typed item (`Type=reasoning` **or** `ResponsesReasoning != nil`), retry once, skip backoff, grant `extraAttempts` so default `MaxRetries=0` still retries. Does not consume the retry budget for transient faults. Does not mutate caller-owned slices.
3. OpenCode Go/Zen egress (`ToOpenAIResponsesRequest`): drop reasoning item ids on the outbound copy. Official OpenAI still receives recovered embedded ids so `encrypted_content` continues to match.

**What it does not do (cache / neighbouring fail-soft):**

- Does not set `BifrostContextKeyBypassSemanticCache` (WebSearch bypass stays search-only).
- Does not touch `prompt_cache_key` / `prompt_cache_retention` / `prompt_cache_options` (that is the Responses→Chat hygiene feature). Nil ids are more prefix-cache stable than a fresh `rs_*` every turn.
- Does not enable `ShouldEmbedReasoningItemID` for OpenCode: a real OpenCode id is also non-replayable under `store:false`.
- Does not strip `encrypted_content` on this 400 (the existing encrypted-content fail-soft is the opposite rewrite and stays gated on `invalid_encrypted_content`).

**Harness exemption:** this is wire-visible, but the public provider-harness collection has no OpenCode Go / gpt-oss Responses host that forwards reasoning content blocks. A case against `openai/gpt-4o-mini` would skip thinking-only items before the wire, so it would be green before and after. Pin with the Go tests in the matrix row; live check is a Claude Code 2-turn tool call on `opencode-go/muse-spark-...` after deploy.

### Claude Code Anthropic conversion mints deterministic function_call item ids (fork feature)

Lets Claude Code keep DeepSeek (and other exact-match prefix-cache) hits on official OpenCode Go (`opencode-go/...` over `/anthropic/v1/messages` → `/v1/responses`). Added because Anthropic→Responses conversion minted a new `fc_*` + 50 random chars as the `function_call` **item** `id` on every history replay. `call_id` (the Anthropic `tool_use` id) was already stable. OpenCode DeepSeek treats the whole input as a prefix key and stops caching at the first reminted `fc_*`; muse-spark ignores those ids and still caches. Unlike `rs_*`, reminted `fc_*` does not 400 on OpenCode (`store:false`) — the requests succeed with ~1% cache.

An earlier version of this feature only omitted those ids on OpenCode egress. Production logs after that deploy still showed a fresh `fc_*` at input index 5 every turn (`call_id` unchanged), and `cached_read` stayed ~2k / ~1%. The omit never changed the in-process request the logging plugin stores, and live cache did not grow. The fix is at the mint site.

**Where it runs:** Anthropic→Responses conversion (`anthropicToolUseBlockToResponsesMessage` and the ungrouped twin). Item id is `fc_` + sha256(call_id) hex[:50] when `isOutputMessage` and call_id is non-empty; otherwise nil. OpenCode Go/Zen egress still drops **reasoning** item ids only, so the stable function_call id reaches OpenCode. Native OpenAI receives the same deterministic id (already not an OpenAI-issued handle).

**What it does not do:**

- Does not strip `call_id`, name, arguments, message ids, or `function_call_output` item ids.
- Does not drop function_call item ids on OpenCode egress (that was the failed approach).
- Does not set `BifrostContextKeyBypassSemanticCache` (WebSearch bypass stays search-only).
- Does not touch `prompt_cache_key` / `prompt_cache_retention` / `prompt_cache_options` (that is the Responses→Chat hygiene feature).
- Does not enable `ShouldEmbedReasoningItemID` for OpenCode.
- Does not rewrite mid-conversation `role:system` on the OpenCode Responses path.

**Harness exemption:** this is wire-visible, but the public provider-harness collection has no OpenCode Go Responses host. Pin with `TestConvertAnthropicToolUse_FunctionCallItemIDIsStable` and `TestToOpenAIResponsesRequest_KeepsFunctionCallItemIDsForOpenCode`. Live check after deploy: same `call_id` must reuse the same `function_call.id` in `logs.db`, and `cached_read` on `opencode-go/deepseek-v4-flash` must grow past ~2k. muse-spark 2-turn tool calls must still succeed.
